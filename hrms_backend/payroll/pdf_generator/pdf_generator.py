...
# Client-selectable payslip designs — each module exposes the same public
# API (generate_payslip_pdf, _build_pdf_bytes) as this file, just with a
# different visual layout. Imported lazily-by-name here (not at call time)
# so a broken design module fails fast at startup rather than mid-request.
from . import design_1, design_2, design_3, design_4, design_5, design_6, design_7, design_8
...
def _build_pdf_bytes(record: PayslipRecord, encryption=None) -> bytes:  # noqa: ARG001
    ...
    # Fitted header text — see text_fit.py. Long client names shrink inside
    # their own box instead of colliding with the right-hand SALARY SLIP block.
    _right_block_w = max(
...
def generate_payslip_pdf(record: PayslipRecord) -> str:
    ...
    design 1 — is generated by its own module. Falls back to design 1 for
